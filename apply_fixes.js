...
const badGridHtml = `                    let styleStr = \`padding: 5px; cursor: pointer; border-radius: 4px; transition: 0.2s; color: \${textColor};\`;
                    if (isSelected) {
...
const goodGridHtml = `                    let styleStr = 'padding: 5px; cursor: pointer; border-radius: 4px; transition: 0.2s; color: ' + textColor + ';';
                    if (isSelected) {
                        styleStr += ' background: var(--primary-light-blue); color: white; font-weight: bold; border: 1px solid var(--primary-light-blue);';
                    } else if (isToday) {
                        styleStr += ' border: 1px solid var(--primary-light-blue); color: var(--primary-light-blue); font-weight: bold;';
...
code = code.replace(badGridHtml, goodGridHtml);

// 2. Fix authenticateOwnerDetailed session bug
const badAuth = `        async function authenticateOwnerDetailed(event) {
            event.preventDefault();
            const bName = document.getElementById('owner-building-name').value;`;

const goodAuth = `        async function authenticateOwnerDetailed(event) {
            event.preventDefault();
...
code = code.replace(badAuth, goodAuth);

// 3. Fix submitContractDetails session bug
const badSubmit = `        async function submitContractDetails(event) {
            event.preventDefault();
...
const goodSubmit = `        async function submitContractDetails(event) {
            event.preventDefault();
            const p = window.pendingContractAuth;
            if (!p) {
                showModalAlert('계약서 인증 정보가 없습니다. 처음부터 다시 시도해주세요.');
...
const badHtml = `                    <p style="font-size: 13px; color: #718096; margin-bottom: 15px;">
                        계약서 이미지를 확인하며 아래 정보를 채워주세요. (일부 정보는 자동 입력될 수 있습니다)
                    </p>
                    
                    <div style="margin-bottom: 20px; text-align: center; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; max-height: 400px; display: flex; justify-content: center; align-items: center; background-color: #f8fafc;">
                        <img id="contract-preview-img" src="" style="max-width: 100%; max-height: 400px; object-fit: contain;">
                    </div>`;

const goodHtml = `                    <p style="font-size: 13px; color: #718096; margin-bottom: 15px;">
...
const badServerApi = `      res.end(JSON.stringify({ success: true }));
  });
  } else if (req.method === 'POST' && parsedUrl.pathname === '/api/verify-contract-ocr') {`;

code = code.replace(badServerApi, serverApi);
