...
// 1. Fix authenticateOwnerDetailed block
const badAuthBlock = `                        if (result.matched) {
...
const goodAuthBlock = `                        if (result.matched) {
                            ...
                            const { data: existingBuildings, error: myCheckError } = await supabaseClient
                                .from('buildings')
                                .select('*')
...
code = code.replace(badAuthBlock, goodAuthBlock);

// 2. Fix OCR UI and Logic
