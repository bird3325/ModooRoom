const fs = require('fs');
let lines = fs.readFileSync('server.js', 'utf8').split('\n');

// 1. ADD ADMIN USERS APP HTML
// We find <div id="map-app" class="hidden">
const mapAppIdx = lines.findIndex(l => l.includes('id="map-app"'));

// Find "전체 회원 관리" card to move
const usersTitleIdx = lines.findIndex(l => l.includes('전체 회원 관리'));

let cardStart = usersTitleIdx;
while(cardStart > 0 && !lines[cardStart].includes('<div class="card">')) {
    cardStart--;
}

let cardEnd = cardStart + 1;
let openDivs = 1;
while(cardEnd < lines.length && openDivs > 0) {
    if (lines[cardEnd].includes('<div ') || lines[cardEnd].includes('<div>')) openDivs += (lines[cardEnd].match(/<div/g) || []).length;
    if (lines[cardEnd].includes('</div>')) openDivs -= (lines[cardEnd].match(/<\/div>/g) || []).length;
    if (openDivs <= 0) break;
    cardEnd++;
}

// Extract the card
const cardLines = lines.splice(cardStart + 1, cardEnd - cardStart - 1); // remove inner content
const placeholderCard = [
    '                <div class="card-title"><i class="fa-solid fa-chart-pie"></i> 관리자 대시보드</div>',
    '                <div style="padding: 40px 20px; text-align: center; color: #718096;">',
    '                    <i class="fa-solid fa-layer-group" style="font-size: 48px; color: #cbd5e0; margin-bottom: 20px;"></i>',
    '                    <h3 style="margin-bottom: 10px; color: #4a5568;">환영합니다, 관리자님!</h3>',
    '                    <p style="margin: 0; line-height: 1.6;">우측 상단의 메뉴를 클릭하여 <strong>회원 관리</strong> 또는 <strong>시스템 설정</strong> 메뉴로 이동하실 수 있습니다.</p>',
    '                </div>'
];
lines.splice(cardStart + 1, 0, ...placeholderCard);

// Create admin-users-app HTML
const adminUsersAppHtml = `
    <div id="admin-users-app" class="hidden">
        <nav class="navbar" style="background: #2d3748;">
            <div class="navbar-brand" style="color: white; cursor: pointer;" onclick="showView('admin-app')">
                <i class="fa-solid fa-user-shield"></i>
                <span style="margin-left:5px;">모두의 방 <span style="font-size:12px; color:#a0aec0;">[관리자 전용 - 회원 관리]</span></span>
            </div>
            <div class="user-profile" style="position: relative;">
                  <button id="admin-users-display-name" class="btn-logout" style="border:none; font-weight: 500; font-size: 14px; cursor: pointer; display: flex; align-items: center; gap: 5px; color: white; background: transparent;" onclick="toggleAdminUsersMenu(event)">
                      관리자 메뉴 <i class="fa-solid fa-chevron-down" style="font-size: 10px;"></i>
                  </button>
                  <div id="admin-users-dropdown" class="dropdown-menu hidden" style="position: absolute; top: 100%; right: 0; background: white; border: 1px solid #e2e8f0; border-radius: 8px; box-shadow: var(--card-shadow); padding: 8px 0; min-width: 150px; z-index: 1000; margin-top: 10px;">
                      <a href="#" class="dropdown-item" onclick="showView('admin-app'); document.getElementById('admin-users-dropdown').classList.add('hidden');" style="color: #4a5568;">관리자 대시보드</a>
                      <hr style="margin: 5px 0; border: none; border-top: 1px solid #e2e8f0;">
                      <a href="#" class="dropdown-item" onclick="showView('admin-users-app'); document.getElementById('admin-users-dropdown').classList.add('hidden');" style="color: #4a5568;">회원 관리</a>
                      <hr style="margin: 5px 0; border: none; border-top: 1px solid #e2e8f0;">
                      <a href="#" class="dropdown-item" onclick="showView('admin-settings-app'); document.getElementById('admin-users-dropdown').classList.add('hidden');" style="color: #4a5568;">시스템 설정</a>
                      <hr style="margin: 5px 0; border: none; border-top: 1px solid #e2e8f0;">
                      <a href="#" class="dropdown-item" onclick="logout(); document.getElementById('admin-users-dropdown').classList.add('hidden');" style="color: #e53e3e;">로그아웃</a>
                  </div>
              </div>
        </nav>
        <div class="main-container">
            <div class="card">
${cardLines.join('\n')}
            </div>
        </div>
    </div>
`.split('\n');

const newMapAppIdx = lines.findIndex(l => l.includes('id="map-app"'));
lines.splice(newMapAppIdx, 0, ...adminUsersAppHtml);

// 2. UPDATE DROPDOWNS
const adminDropdownStr = `<hr style="margin: 5px 0; border: none; border-top: 1px solid #e2e8f0;">
                      <a href="#" class="dropdown-item" onclick="showView('admin-users-app'); document.getElementById('admin-dropdown').classList.add('hidden');" style="color: #4a5568;">회원 관리</a>`;
const adminSettingsDropdownStr = `<hr style="margin: 5px 0; border: none; border-top: 1px solid #e2e8f0;">
                      <a href="#" class="dropdown-item" onclick="showView('admin-users-app'); document.getElementById('admin-settings-dropdown').classList.add('hidden');" style="color: #4a5568;">회원 관리</a>`;

for(let i = 0; i < lines.length; i++) {
    if (lines[i].includes(`onclick="showView('admin-settings-app'); document.getElementById('admin-dropdown')`)) {
        if (!lines[i-1].includes('회원 관리') && !lines[i-2].includes('회원 관리')) {
            lines.splice(i, 0, ...adminDropdownStr.split('\n'));
            i += 2;
        }
    }
    if (lines[i].includes(`onclick="showView('admin-settings-app'); document.getElementById('admin-settings-dropdown')`)) {
        if (!lines[i-1].includes('회원 관리') && !lines[i-2].includes('회원 관리')) {
            lines.splice(i, 0, ...adminSettingsDropdownStr.split('\n'));
            i += 2;
        }
    }
}

// 3. UPDATE showView
const showViewIdx = lines.findIndex(l => l.includes(`if(document.getElementById('admin-settings-app')) document.getElementById('admin-settings-app').classList.add('hidden');`));
if (showViewIdx !== -1) {
    lines.splice(showViewIdx + 1, 0, `            if(document.getElementById('admin-users-app')) document.getElementById('admin-users-app').classList.add('hidden');`);
}

for(let i=0; i < lines.length; i++) {
    if(lines[i].includes(`} else if (viewName === 'admin-app') {`)) {
        const insertion = `            } else if (viewName === 'admin-users-app') {
                document.getElementById('admin-users-app').classList.remove('hidden');`;
        lines.splice(i, 0, ...insertion.split('\n'));
        break;
    }
}

// 4. UPDATE JS CLICK LOGIC
const toggleAdminSettingsIdx = lines.findIndex(l => l.includes('function toggleAdminSettingsMenu(e)'));
if (toggleAdminSettingsIdx !== -1) {
    const toggleFunc = `        function toggleAdminUsersMenu(e) {
            e.stopPropagation();
            document.getElementById('admin-users-dropdown').classList.toggle('hidden');
        }`;
    lines.splice(toggleAdminSettingsIdx, 0, ...toggleFunc.split('\n'), '');
}

const outsideClickIdx = lines.findIndex(l => l.includes("const adminSettingsDropdown = document.getElementById('admin-settings-dropdown');"));
if (outsideClickIdx !== -1) {
    const clickLogic = `            const adminUsersDropdown = document.getElementById('admin-users-dropdown');
            const adminUsersToggleBtn = document.getElementById('admin-users-display-name');
            if (adminUsersDropdown && adminUsersToggleBtn && !adminUsersToggleBtn.contains(e.target) && !adminUsersDropdown.contains(e.target)) {
                adminUsersDropdown.classList.add('hidden');
            }`;
    lines.splice(outsideClickIdx, 0, ...clickLogic.split('\n'), '');
}

fs.writeFileSync('server.js', lines.join('\n'), 'utf8');
console.log('Setup complete!');
