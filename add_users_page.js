...
// The admin-app starts at:
// <div id="admin-app" class="hidden">
// inside it, there is a nav, then:
...
//         </div>
//     </div>
// Next sibling is usually <div id="map-app" class="hidden"> or similar.
...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    // Replace the dropdown links in adminAppSection (to add "회원 관리")
    const oldDropdown = `<a href="#" class="dropdown-item" onclick="showView('admin-app'); document.getElementById('admin-dropdown').classList.add('hidden');" style="color: #4a5568;">관리자 대시보드</a>
    ...
    adminAppSection = adminAppSection.replace(oldDropdown, newDropdown);

    // Extract the card content
    const cardStartMatch = adminAppSection.match(/<div class="card">\s*<div class="card-title"><i class="fa-solid fa-users-gear"><\/i> 전체 회원 관리<\/div>/);
    if (!cardStartMatch) {
        // Look for the generic one
        const genericMatch = adminAppSection.indexOf('<div class="card">');
        if (genericMatch !== -1) {
            ...
            if (match) {
                ...
                // Create placeholder card for admin-app
...
// Also update admin-settings-app dropdown
const oldSettingsDropdown = `<a href="#" class="dropdown-item" onclick="showView('admin-app'); document.getElementById('admin-settings-dropdown').classList.add('hidden');" style="color: #4a5568;">관리자 대시보드</a>
                      <hr style="margin: 5px 0; border: none; border-top: 1px solid #e2e8f0;">
...
content = content.replace(oldSettingsDropdown, newSettingsDropdown);
...
content = content.replace(`} else if (viewName === 'admin-settings-app') {
...
console.log('Successfully added admin-users-app');
