...
#### 레이아웃 및 디자인 통일 사양
- **통합 레이아웃 구조**: 호실 추가, 건물 추가, 2차 인증 성공 시, 별도의 성공 안내 모달창을 띄우지 않고 기존에 존재하는 **"호실 및 임대차 상세 정보 수정" 페이지(`room-detail-page`)를 그대로 직접 호출**하여 100% 동일한 디자인과 사용자 경험을 제공합니다.
- **호실 추가 프로세스 규격**:
  * 임대인이 특정 건물의 호실을 추가하면, DB/로컬 캐시에 즉시 '공실' 상태의 기본 호실 레코드를 생성합니다.
  * 생성 완료 알림창(모달 팝업)을 띄우지 않고, 즉시 `openRoomDetailPage(bIdx, rIdx)`를 호출하여 해당 호실의 수정 페이지로 바로 이동합니다.
- **연동 및 저장 정책 동기화**:
  * 2차 인증(계약서 인증) 및 신규 건물 등록 성공 시 백그라운드에서 즉시 Gemini API를 활용한 상세 OCR 분석을 실행합니다.
