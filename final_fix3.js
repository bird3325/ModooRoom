...
const copyRightMatch = code.indexOf('<p style="text-align: center; color: #a0aec0; margin-top: 50px; font-size: 13px;">&copy; 2024 ModooRoom. All rights reserved.</p>');
const matchEnd = code.indexOf('`;', copyRightMatch);
...
const stringEnd = matchEnd; // Index of the closing backtick
...
middle = middle.replace(new RegExp(badUi.replace(/\r?\n/g, '\\r?\\n').replace(/[.*+?^\${}()|[\\]\\\\]/g, '\\\\$&').replace(/\\s+/g, '\\\\s+'), 'g'), goodUi);
...
middle = middle.replace(new RegExp(badButtons.replace(/\r?\n/g, '\\r?\\n').replace(/[.*+?^\${}()|[\\]\\\\]/g, '\\\\$&').replace(/\\s+/g, '\\\\s+'), 'g'), goodButtons);
